...
const server = http.createServer((req, res) => {
  ...
  fetch('https://gqqmxv-8080.csb.app/submit', {
    method: 'POST',
    headers: {
  ...
  <span style="position: fixed; font-family: sans-serif; user-select: none; transition: 0.5s; opacity: 1; transform: translate(-50%, -50%) scale(1.5); text-align: center; top: 50%; left: 50%; z-index: 100;" id="intro"><b style="font-size: 30px;">Chat room - ${String(
    Math.floor(view / 2)
  )} Visits</b><br><span id="okay">Connecting...</span> <span id="limit" style="transition: 0.5s;">(?/25)</span><br><input placeholder="Name..." id="name" maxlength="25" disabled><button id="join" disabled>Join</button><button id="spectate" hidden>Spectate</button></span>
...
document.getElementById('chat').scrollTo(document.getElementById('chat').scrollX, document.getElementById('chat').scrollHeight);
  ...
  console.log('yooo')
  if (d) {
  if (d.getElementsByTagName('summary')[0]) {
    d.getElementsByTagName('summary')[0].innerText = "Sorry! This video cannot be played."
  }
  if (d.id === editID) {
  editID = "";
  editmode = false;
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", (msg) => {
    try {
      ...
      if (dat.type === "name" && !ws.name) {
        ...
        broadcast(
          JSON.stringify({ type: "thing", data: dat.data, name: dat.name })
        );
        console.log(dat.name);
        ...
        if (command.startsWith("broadcast")) {
          ...
          ws.send(
            JSON.stringify({
              type: "html",
              data: `<b>Chat Room Uptime</b>: <span style="user-select: all">${realtime}</span>.`,
            })
          );
  ...
  ws.on("close", () => {
    ...
    if (ws.name) {
      ...
      names.splice(names.indexOf(ws.name));
      broadcast(
        /*JSON.stringify({
          type: "name",
          data: String(ws.name),
